...
{
    head = NULL; // Sets Node head to null since list is just created there are currently no Nodes in the list.
}
LinkedList::~LinkedList()
{
    clearList();
...
void LinkedList::printList(bool flag) // if no flag is passed it defaults to false
{
    ...
    if(flag && head != NULL)
    {
        while(current->next != NULL) // Sets node to last node
        {
            current = current->next;
        }
        while(current != NULL)
